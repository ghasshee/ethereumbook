...
セキュリティはスマートコントラクトを書く際、最も気をつけるべきことの一つである。スマートコントラクトのプログラミングの世界では、間違いを犯すと簡単に発見され、ひどい痛手を追うことになる。
...
7. _EtherStore.sol_, line 11: この２回目の `withdrawFunds` を呼び出しする際、攻撃コントラクトの残高はまだ `1 ether` で、18行目はまだ実行されていない。
このようにして、`balances[0x0..123] = 1 ether` とういう状態が持続するのだ。
これは `lastWithdrawTime` についても同じことが言える。そうして再度すべての要求条件をみたすことになるのだ。
